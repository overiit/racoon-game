...
import { GroupToMeshes } from './Convert'
...
class CannonUtils {
    public static GroupToCannon(group: THREE.Group, baseBody: CANNON.Body): CANNON.Body {
        const meshes = GroupToMeshes(group)
        for (const mesh of meshes) {
            baseBody.addShape(this.CreateTrimesh(mesh.geometry));
        }
        return baseBody;
    }

    public static CreateTrimesh(geometry: THREE.BufferGeometry, offset: Vec3=new Vec3(0, 0, 0)): CANNON.Trimesh {
        const vertices = (geometry.clone().toNonIndexed().attributes.position.array as number[]).map(r => parseFloat(r.toFixed(5)))
        // offset
        for (let indeces = 0; indeces < vertices.length; indeces += 3) {
            vertices[indeces] += offset.x
            vertices[indeces + 1] += offset.z
            vertices[indeces + 2] += offset.y
        }
        
        const indices = Object.keys(geometry.index.array).map(Number)
